...
# pipes translate \n to \r\n and stdout can default to cp1252. The prompt is
# fed via stdin, never argv — Git Bash path-converts arguments starting with
# "/" when invoking native exes, which would mangle "/queue ..." prompts.
mkin() {
  printf '%s' "$1" | python3 -c '
import json, sys
prompt = sys.stdin.buffer.read().decode("utf-8")
sys.stdout.buffer.write(json.dumps(
    {"session_id": sys.argv[1], "transcript_path": "/x.jsonl",
     "cwd": "/tmp", "hook_event_name": "UserPromptSubmit",
     "prompt": prompt}, separators=(",", ":"), ensure_ascii=False).encode("utf-8"))' "$SID"
}
